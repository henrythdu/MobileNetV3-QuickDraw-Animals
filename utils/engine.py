...
def train_step(model: torch.nn.Module, 
    ...
    train_loss = 0
    train_batch_correct = 0
    train_batch_correct_top5 = 0
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        batch_acc_info = accuracy_tracking_per_batch(y_pred,y, topk=topk)
...
def test_step(model: torch.nn.Module, 
    ...
    test_loss = 0
    test_batch_correct = 0
    test_batch_correct_top5 = 0
    test_batch_length = 0
    topk = (1,5)
    ...
    with torch.inference_mode():
        ...
        for batch, (X, y) in enumerate(dataloader):
            ...
            batch_size = len(y_preds)
            batch_acc_info = accuracy_tracking_per_batch(y_preds,y, topk=topk)
            test_batch_correct += batch_acc_info[0]
            test_batch_correct_top5 +=batch_acc_info[1]
            test_batch_length += batch_size
    # Adjust metrics to get average loss and accuracy per batch 
    test_acc = test_batch_correct / test_batch_length
